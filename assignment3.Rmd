...
author: "Olivia Hudson Candidate Number: 089369 "
date: "12/02/2020"
...
**(NOTE: i have recoded the waves earlier so instead of alphabetical letters they are properly named by wave numbers.)**

Write a short interpretation of your findings.

For the vote6 variable; an answer of 2 means that they are 'Fairly interested' in politics and an answer of 3 means that they are 'Not very interested' in politics.

From these findings we can see that:
- the average political interest for females in the data set is around 2.8
 - this suggests that females in the dataset are Fairly interested in politics
- the average political interest for males in the data set is around 2.5
 - this suggests that males in the dataset are also fairly interested in politics, but       more so than women
This suggest that men in the data set are, on average, more interested in politics than the females in the data set.
This is fairly consistent across the waves, but with the lowest values being at wave 7.
...
LongDelta <- Long %>%
  ...
  mutate(Delta = na.omit(("wave 2" - "wave 1") + ("wave 3" - "wave 2") + ("wave 4" - "wave 3") + ("wave 5" - "wave 4") + ("wave 6" - "wave 5") + ("wave 7" - "wave 6")))
  group_by(sex_dv) %>%
  summarise(MeanDeltaSex = mean(Delta)) %>%
  group_by(age)
